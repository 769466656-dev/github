...
输入：/Volumes/拍摄02/沪深A股_Tushare/03_年报财务数据/年报_2025/fina_indicator_筛选1.csv
输出：data/stock_fina_selected_Tushare.csv
...
DEFAULT_INPUT_FILE = "/Volumes/拍摄02/沪深A股_Tushare/03_年报财务数据/年报_2025/fina_indicator_筛选1.csv"
# 如需临时改用其他文件，可设置环境变量 FINA_INDICATOR_FILE。
INPUT_FILE = os.environ.get("FINA_INDICATOR_FILE", DEFAULT_INPUT_FILE)
OUTPUT_FILE = os.path.join(DATA_DIR, "stock_fina_selected_Tushare.csv")
...
def main():
    ...
    if df.empty:
        print("错误：输入文件只有表头，尚未包含任何 Tushare 财务指标数据。")
        return

    # Tushare fina_indicator 使用 ocf_to_or；筛选脚本统一采用更易读的字段名。
    df = df.rename(columns={'ocf_to_or': 'ocf_to_revenue'})
    ...
    if len(selected) > 0:
        ...
        print("达标股票（按 ROE 排序）：")
        print("=" * 60)
        disp_cols = ['stock_code', 'ts_code', 'stock_name', 'end_date', 'roe', 'netprofit_yoy', 'grossprofit_margin',
